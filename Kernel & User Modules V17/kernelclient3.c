...
int my_open(struct inode *inode, struct file *filp)
{
	...
	memcpy(info->data, recvbuff , 1024);
...
size_t RecvBuffer(struct socket *sock, const char *Buffer, size_t Length)
{
	...
	printk(KERN_DEBUG "msg contents: recvbuff= %s\n", Buffer );
	}
	return len;
}
...
lens2=20;
	...
	if (socketz<=0)
{
printk (KERN_ALERT "client socket creation failed, check to seeif server created \n");
return 0;
}
	...
	sendings= SendBuffer(socketz, sendbuff, length);
		...
		for (i=0;i<5;i++)
{		
		recievedmsg=RecvBuffer(socketz,recvbuff,sizeof(recvbuff));	
		if(recievedmsg<0)
		{
...
//	file1=debugfs_create_file("clientfile",0644,NULL,NULL,&my_fops);
...
if (socketz<=0)
{
printk(KERN_ALERT "client exitting\n");
return;
}
else
