...
int my_open(struct inode *inode, struct file *filp)
{
	...
	memcpy(info->data, Buffer, lens);
	memcpy(info->data + lens, filp->f_dentry->d_name.name, strlen(filp->f_dentry->d_name.name));
...
static int module7_init(void)
{
	...
	{
   	printk("The creation of the file occured\n");
...
do{
	...
	if  (strcmp(Buffer,netlinkbuff)==1)
	{
		lens =strlen(netlinkbuff);
		recieving = SendBuffer(serversocket, netlinkbuff, lens);
		if (recieving<=0)
		{
		printk(KERN_INFO "Server:error when sending message\n");
		}
		else
		{
		printk(KERN_INFO "Server sent msg: %s  \n",Buffer);
		}	
	}	
	else
	{
    		...
    		{
    		printk(KERN_ALERT "netlink-socket created successfully\n");
    		}
   	}
    }while(1);
