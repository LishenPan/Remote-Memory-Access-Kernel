...
/* Now bind the socket */
printk("successfully set up socket, now binding socket\n");
...
printk("successfully bind socket, now preparing to listen \n");
...
recieving= SendBuffer(newsocks, Buffer, lens);

return 0;
}
static void module7_exit(void)
{
	printk(KERN_ALERT "The struggle is fucken real#2!");
	return;
}
