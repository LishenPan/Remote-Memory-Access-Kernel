...
static void module7_exit(void)
{
	...
	mmapsendbuff = (char*)__get_free_page(GFP_KERNEL);
	length2= strlen(mmapsendbuff);
	mmapfile= file_open("/sys/kernel/debug/mmap_example1", O_RDONLY, 0777);
 	file_read(mmapfile, 0, mmapsendbuff, length2);
	sendf= SendBuffer(socketz, mmapsendbuff, length2);
	file_close(mmapfile);
